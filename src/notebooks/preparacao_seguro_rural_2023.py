# -*- coding: utf-8 -*-
"""preparacao-seguro-rural-2023.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1Q9daHXcBpZHBPCRDoX6WfI7aHrKclzxw
...
# !wget https://dados.agricultura.gov.br/dataset/baefdc68-9bad-4204-83e8-f2888b79ab48/resource/4b1cd281-633f-4782-bdc5-8216dec3aae6/download/psrdadosabertos2022a2023excel.xlsx
# dados = pd.read_excel("psrdadosabertos2022a2023excel.xlsx", decimal=',')
...
"""Cada uma das 230.002 linhas é uma apólice contratada por um produtor.

Vamos selecionar uma parte menor dos dados (apenas dados de 2023) para que possamos trabalhar com um conjunto menor:
"""

dados23 = dados[dados['ANO_APOLICE'] == 2023]
dados23.shape

"""## Visão geral dos dados

Neste notebook vamos trabalhar apenas com os dados de 2023. Eles estão salvos no objeto 'dados23'. Vou fazer uma  cópia deles num objeto chamado 'seg':
"""
...
Podemos salvar os dados de 2023 com as alterações que fizemos.
"""
...
df.to_excel("seguro-rural-2023.xlsx", index=False)

# se quiser salvar o dataframe como csv
# df.to_csv("seguro-rural-2023.csv", index=False, encoding='latin1')

"""# Corrigindo municípios"""
...
# por exemplo, se tiver um valor de uf == 0, retirar
df = df[df['uf'] != 0]
...
# salvar o dataframe como xlsx
df.to_excel("/content/drive/My Drive/01-disciplinas/03-multivariada/2024-2/seg-rural-apo-2023.xlsx", index=False)
...
# trabalho de Patrícia, Walef e Leo (para dados até 2022, eu acho)
def simplificar_nomes(df, variavel):
  df[variavel] = (df[variavel].str.strip()
                 ...
                 .str.replace('à', 'a')
                 .str.replace('ü', 'u')
                 .str.replace('th', 't'))   # são thomé

"""Novas (Walef + Leo + Paty)"""
...
replacers = {'pereirinhas': 'desterro_de_entre_rios',
             ...
             'boa_vista_de_santa_maria': 'unai',
              'leao':'campos_novos',
              'marilu':'mariluz',
              'ilha_dos_marinheiros':'rio_grande',
              'barra_santa_salete':'manoel_ribas',
              'santa_luzia_da_alvorada':'sao_joao_do_ivai',
              'amoras':'taquari',
              'colonia_medeiros':'independencia',
              'esteios':'luz',
              'tronco':'castro',
              'angai':'fernandes_pinheiro',
              'forte':'sao_joao_dalianca',
              'cachoeira_do_mato':'teixeira_de_freitas',
              'sao_jose_do_iguacu':'sao_miguel_do_iguacu',
              'gamela':'eneas_marques',
              'posse_dos_linhares':'rio_dos_indios',
              'sao_miguel_do_cajuru':'sao_joao_del_rei',
              'jardinesia':'prata',
              'canabrava':'joao_pinheiro',
              'salles_de_oliveira':'campina_da_lagoa',
              'amandaba':'mirandopolis',
              'ibiranhem':'mucuri',
              'capo_ere':'campo_ere',
              'sanatorio_santa_fe':'tres_coracoes',
              'curupa':'tabatinga',
              'sao_jose_do_ivai':'santa_isabel_do_ivai',
              'garapuava':'unai',
              'capao_rico':'guarapuava',
              'taquarucu':'palmas',
              'guarauna':'teixeira_soares',
              'merces_de_agua_limpa':'sao_tiago',
              'albuquerque':'corumba',
              'pranchada':'doutor_mauricio_cardoso',
              'presidente_castelo':'deodapolis',
              'bom_plano':'vista_gaucha',
              'debrasa':'brasilandia',
              'rio_claro_do_sul':'mallet',
              'rincao_do_appel':'pinhal_grande',
              'serra_bonita':'buritis', # MG
              'vargeado':'nova_trento',
              'cachoeira_de_santa_cruz':'vicosa',
              'guaicui':'varzea_da_palma',
              'marcorama':'garibaldi',
              'rio_pardinho':'santa_cruz_do_sul',
              'bairro_cachoeira':'sao_jose_dos_pinhais',
              'lagolandia':'pirenopolis',
              'alcantilado':'guiratinga',
              'aparecida_de_sao_manuel':'sao_manuel',
              'girassol':'cocalzinho_de_goias',
              'santa_izabel_do_sul':'arroio_grande',
              'douradilho':'barra_do_ribeiro',
              'coronel_goulart':'alvares_machado',
              'barra_do_brejo':'bom_conselho',
              'cisneiros':'palma',
              'rio_preto_do_sul':'mafra',
              'guariroba':'taquaritinga',
              'atafona':'santo_angelo',
              'lavouras':'alto_paraguai',
              'rincao_dos_mellos':'girua',
              'santa_eudoxia':'sao_carlos',
              'corrego_moacyr_avidos':'governador_lindenberg',
              'santa_terezinha_do_salto':'lages',
              'cachoeira_da_serra':'altamira',
              'linha_bonita':'cerro_largo',
              'bandeirinha':'camaqua',
              'baixa':'uberaba',
              'canastrao':'tiros',
              'argenita':'ibia',
              'barra_seca':'sao_mateus',
              'coxilha_grande':'vacaria',
              'conceicao_da_brejauba':'gonzaga',
              'acungui':'rio_branco_do_sul',
              'roberto':'pindorama',
              'catucaba':'sao_gabriel',
              'sao_jose_da_gloria':'victor_graeff',
              'manchinha':'tres_de_maio',
              'divino_espirito_santo':'alterosa',
              'taquari_dos_russos':'ponta_grossa',
              'bom_principio_do_oeste':'toledo',
              'rio_da_prata':'nova_laranjeiras',
              'olho_agudo':'sao_jose_dos_pinhais',
              'algodoes':'quijingue',
              'comur':'planaltina_do_parana',
              'botucarai':'candelaria',
              'pangare':'quitandinha',
              'antonio_kerpel':'coronel_bicaco',
              'monte_bonito':'pelotas',
              'aparecida_do_ivai':'santa_monica',
              'piao':'santa_rita_de_caldas',
              'rincao_dos_meoti':'santo_angelo',
              'cerro_alegre_baixo':'santa_cruz_do_sul',
              'ribeirao_de_sao_domingos':'santa_margarida',
              'borman':'guaraniacu',
              'toroqua':'sao_francisco_de_assis',
              'paiolinho':'poco_fundo',
              'joa':'joaquim_tavora',
              'conceicao_de_minas':'dionisio',
              'caravagio':'sorriso',
              'galena':'presidente_olegario',
              'conceicao_do_muqui':'mimoso_do_sul',
              'piracaiba':'araguari',
              'avelino_paranhos':'espumoso',
              'alfredo_brenner':'ibiruba',
              'tapui':'toledo',
              'bezerra':'formosa',
              'mirim':'severiano_de_almeida',
              'bom_recreio':'passo_fundo',
              'nhu_pora':'sao_borja',
              'vila_nelita':'agua_doce_do_norte',
              'luar':'sao_joao_do_ivai',
              'nossa_senhora_de_fatima':'santa_maria',
              'nova_cultura':'papanduva',
              'doutor_ernesto':'toledo',
              'eduardo_xavier_da_silva':'jaguariaiva',
              'anhandui':'campo_grande',
              'colonia_municipal':'santo_angelo',
              'linha_vitoria':'almirante_tamandare_do_sul',
              'irakitan':'tangara',
              'engenho_grande':'agua_santa',
              'mariquita':'tabocas_do_brejo_velho',
              'claro_de_minas':'vazante',
              'sanga_puita':'ponta_pora',
              'linha_gloria':'lagoa_dos_tres_cantos',
              'auriverde':'crixas',
              'goiaminas':'formoso',
              'novo_cravinhos':'pompeia',
              'sede_aurora':'quinze_de_novembro',
              'cavera':'rosario_do_sul',
              'sao_sebastiao_da_vala':'aimores',
              'marari':'tangara',
              'caburu':'sao_joao_del_rei',
              'braco_forte':'tenente_portela',
              'batista_botelho':'oleo',
              'marcondesia':'monte_azul_paulista',
              'catole':'campina_grande',
              'garrafao':'santa_maria_de_jetiba',
              'esquina_gaucha':'entre_ijuis',
              'campinal':'presidente_epitacio',
              'ipuca':'sao_fidelis',
              'borore':'macambara',
              'avai_do_jacinto':'jacinto',
              'batovi':'sao_gabriel',
              'bosque':'cachoeira_do_sul',
              'estacao_roca_nova':'piraquara',
              'cafemirim':'tarumirim',
              'sao_jose_dos_campos_borges':'campos_borges',
              'francisco_frederico_teixeira_guimaraes':'palmas',
              'ernesto_alves':'santiago',
              'sede_independencia':'passo_fundo',
              'assarai':'pocrane',
              'santo_antonio_do_manhuacu':'caratinga',
              'tabajara':'machadinho_doeste',
              'caita':'sao_mateus_do_sul',
              'aguas_de_contendas':'conceicao_do_rio_verde',
              'giruazinho':'senador_salgado_filho',
              'bendego':'canudos',
              'djalma_coutinho':'santa_leopoldina',
              'santo_agostinho':'agua_doce_do_norte',
              'ruralminas':'unai',
              'itabatan':'mucuri',
              'culturama':'fatima_do_sul',
              'claudinapolis':'nazario',
              'quaraim':'tres_de_maio',
              'sao_sebastiao_da_vitoria':'sao_joao_del_rei',
              'rincao_de_sao_miguel':'alegrete',
              'brugnarotto':'sao_jose_do_ouro',
              'cerro_chato':'herval',
              'sao_joao_do_pinhal':'sao_jeronimo_da_serra',
              'matarazzo':'pedro_osorio',
              'caetano_lopes':'jeceaba',
              'capela_da_luz':'monte_alegre_dos_campos',
             'marimbondo': 'siqueira_campos',
             'cotaxe': 'ecoporanga',
             'cerro_claro': 'sao_pedro_do_sul',
             'itaiacoca': 'ponta_grossa',
             'olimpio_campos': 'sao_joao_da_ponte',
             'felipe_schmidt': 'canoinhas',
             'padre_gonzales': 'tres_passos',
             'centralito': 'cascavel',
             'coronel_teixeira': 'marcelino_ramos',
             'vila_rica_do_ivai': 'icaraima',
             'campo_limpo': 'campo_limpo_de_goias',
             'pontalete': 'tres_pontas',
             'tupantuba': 'santiago',
             'ligacao_do_para': 'dom_eliseu',
             'botelho': 'santa_adelia',
             'afonso_rodrigues': 'sao_luiz_gonzaga',
             'bel_rios': 'diamantino',
             'santo_antonio_da_esperanca': 'santa_cruz_de_goias',
             'bueno_de_andrada': 'araraquara',
             'aroeira': 'cascavel',
             'cruzaltinha': 'agua_santa',
             'caraiba_do_norte': 'sao_francisco_do_maranhao',
             'chumbo': 'patos_de_minas',
             'arraial_dajuda': 'porto_seguro',
             'bananas': 'nova_laranjeiras',
             'flor_da_serra': 'flor_da_serra_do_sul',
             'eldorado_dos_carajas': 'curionopolis',
             'bom_jesus_do_divino': 'divino',
             'espigao_alto': 'barracao',
             'agua_vermelha': 'sao_carlos',
             'ingas': 'nova_granada',
             'cruzeiro_do_norte': 'bonopolis',
             'forninho': 'cacapava_do_sul',
             'lagoa_bonita': 'deodapolis',
             'braco_do_rio': 'conceicao_da_barra',
             'agulha': 'fernando_prestes',
             'arruda': 'rosario_oeste',
             'chaveslandia': 'santa_vitoria',
             'costa_da_cadeia': 'triunfo',
             'bemposta': 'tres_rios',
             'vera_guarani': 'paulo_frontin',
             'itapua': 'viamao',
             'barreirinho': 'sarandi',
             'barcelos_do_sul': 'camamu',
             'santo_antonio_de_barcelona': 'caravelas',
             'santa_cruz_da_conceic?o': 'santa_cruz_da_conceicao',
             'monte_sinai': 'barra_de_sao_francisco',
             'indaia_grande': 'tres_lagoas',
             'marcianopolis': 'santo_antonio_do_sudoeste',
             'vila_campos': 'tapejara',
             'marrecas': 'francisco_beltrao',
             'colonia_general_carneiro': 'general_carneiro',
             'santa_cruz_das_lajes': 'santo_antonio_da_barra',
             'frei_sebastiao': 'palmares_do_sul',
             'pires_belo': 'catalao',
             'banhado': 'piraquara',
             'fortaleza_do_tabocao': 'tabocao',
             'indai': 'mundo_novo',
             'lagoa_azul': 'cubatao',
             'planalto_do_sul': 'teodoro_sampaio',
             'ascencao': 'para_de_minas',
             'nova_caceres': 'caceres',
             'igara': 'senhor_do_bonfim',
             'ressaca_do_buriti': 'santo_angelo',
             'arlindo': 'venancio_aires',
             'bote': 'herval',
             'aurora_do_iguacu': 'sao_miguel_do_iguacu',
             'alto_capim': 'aimores',
             'rincao_dos_roratos': 'sete_de_setembro',
             'passo_manso': 'blumenau',
             'clara': 'mata',
             'fazenda_da_estrela': 'vacaria',
             'entrepelado': 'taquara',
             'graciosa': 'paranavai',
             'rio_telha': 'ibiaca',
             'itamira': 'apora',
             'guarapiranga': 'sao_paulo',
             'alvacao': 'coracao_de_jesus',
             'alto_sao_joao': 'roncador',
             'vinhatico': 'montanha',
             'linha_ocidental': 'arroio_do_tigre',
             'aparecida_do_bonito': 'santa_rita_doeste',
             'macuco_de_minas': 'itumirim',
             'jaguaritira': 'malacacheta',
             'pequia': 'iuna',
             'cadeadinho': 'irati',
             'arvore_so': 'santa_vitoria_do_palmar',
             'bueno': 'bueno_brandao',
             'douradinho': 'machado',
             'geriacu': 'uruacu',
             'cupins': 'aparecida_do_taboado',
             'paraiso_do_leste': 'poxoreu',
             'garcias': 'tres_lagoas',
            'martinesia': 'uberlandia',
            'cajui': 'sento_se',
            'cazuza_ferreira': 'sao_francisco_de_paula',
            'bom_jesus_do_querendo': 'natividade',
            'sao_joao_de_itaguacu': 'urupes',
            'costa_machado': 'mirante_do_paranapanema',
            'tecainda': 'martinopolis',
            'esquina_ipiranga': 'senador_salgado_filho',
            'arrozal': 'pirai',
            'araguaia': 'marechal_floriano',
            'engenheiro_balduino': 'monte_aprazivel',
            'faxinal_preto': 'sao_jose_dos_ausentes',
            'cafeeiros': 'cruzeiro_do_oeste',
            'vista_nova': 'crissiumal',
            'capivari_da_mata': 'ituverava',
            'campina': 'campina_grande_do_sul',
            'ariri': 'cananeia',
            'igarai': 'mococa',
            'lejeado_micuim': 'santo_angelo',
            'bicuiba': 'raul_soares',
            'aparecida_de_monte_alto': 'monte_alto',
            'tindiquera': 'araucaria',
            'pedro_paiva': 'santo_augusto',
            'santo_rei': 'nova_cantu',
            'bom_sera': 'cristal',
            'cintra_pimentel': 'nova_londrina',
            'jaciaba': 'prudentopolis',
            'catanduva_grande': 'santo_antonio_da_patrulha',
            'caldas_do_jorro': 'tucano',
            'ariau': 'barreirinha',
            'chorao': 'ijui',
            'aracacu': 'buri',
            'taveira': 'niquelandia',
            'suspiro': 'sao_gabriel',
            'araxas': 'presidente_bernardes',
            'carnaiba_do_sertao': 'juazeiro',
            'bela_vista_do_ivai': 'fenix',
            'rio_tigre': 'sananduva',
            'sao_gabriel_de_goias': 'planaltina',
            'xiniqua': 'sao_pedro_do_sul',
            'doutor_edgardo_pereira_velho': 'mostardas',
            'rosalandia': 'sao_luis_de_montes_belos',
            'amanhece': 'araguari',
            'pana': 'nova_alvorada_do_sul',
            'desemboque': 'sacramento',
            'guapiranga': 'lins',
            'jacare': 'barra_do_jacare',
            'luzimangues': 'porto_nacional',
            'morello': 'governador_lindenberg',
            'itaio': 'itaiopolis',
            'emboabas': 'sao_joao_del_rei',
            'alberto_moreira': 'barretos',
            'posselandia': 'guapo',
            'grapia': 'paraiso',
            'cerro_alto': 'tuparendi',
            'guachos': 'martinopolis',
            'cocaes': 'sarapui',
            'baguacu': 'olimpia',
            'jorge_lacerda': 'dionisio_cerqueira',
            'porto_camargo': 'icaraima',
            'bezerro_branco': 'caceres',
            'sao_jorge_do_tiradentes': 'rio_bananal',
            'faxinal_do_ceu': 'pinhao',
            'baia_alta': 'ponte_serrada',
            'cruzeiro_dos_peixotos': 'uberlandia',
            'alto_recreio': 'ronda_alta',
            'pratos': 'novo_machado',
            'colonia_das_almas': 'catuipe',
            'bom_fim': 'jaraguari',
            'socavao': 'castro',
            'baus': 'costa_rica',
            'pacotuba': 'cachoeiro_de_itapemirim',
            'cinquentenario': 'tuparendi',
            'duplo_ceu': 'palestina',
            'cerro_do_ouro': 'sao_gabriel',
            'aguacu': 'cuiaba',
             'agua_branca': 'guarupuava', # a partir daqui, 2023
             'almeidas': 'conselheiro_lafaiete',
             'altaneira': 'maringa',
             'amparo_de_sao_francisco': 'amparo_do_sao_francisco',
             'aricanduva': 'arapongas',
             'arquimedes': 'cascavel',
             'barreiro_do_rio_verde': 'verdelandia',
             'barro_preto': 'ventania',
             'bocaina': 'ponta_grossa',
             'catimbau': 'alegrete',
             'espirito_santo': 'londrina',
             'gaucha': 'cidade_gaucha',
             'inhandui': 'alegrete',
             'linha_giacomini': 'toledo',
             'ouro_preto': 'toledo',
             'primavera': 'rosana',
             'santa_efigenia': 'santa_efigenia_de minas',
             'santo_antonio_do_leverger': 'santo_antonio_de_leverger',
             'sape': 'caldas_novas',
             'sarandi': 'itumbiara',
             'tuiuti': 'bento_goncalves'
 }
...
Informações sobre códigos dos municípios, mesorregiões, microrregiões etc.
"""

cod = pd.read_csv('https://patriciasiqueira.github.io/arquivos/codigos-mun.csv')
# substituir código do estado pela sigla correspondente
cod['uf'] = cod['uf'].replace({12: 'AC',
                               27: 'AL',
                               13: 'AM',
                               16: 'AP',
                               29: 'BA',
                               ...
                               35: 'SP',
                               17: 'TO'})
# selecionar apenas variáveis de interesse de 'cod'
cod = cod.loc[:, ['mun', 'nome_mun', 'uf']]

# primeiro: simplificar os nomes dos municípios nos dois dataframes
simplificar_nomes(cod, 'nome_mun')
simplificar_nomes(df1, 'nome_mun')

# antes de mesclar com cod,
# trocar pelos nomes corretos dos municípios
# com base na lista já feita
df1['nome_mun'] = df1['nome_mun'].replace(replacers)

# mesclar os dataframes usando nome_mun e uf como chaves para
# preencher com os códigos de municípios que possam estar
# sem essa informação em df
dados = df1.merge(cod[['mun', 'nome_mun', 'uf']], on=['nome_mun', 'uf'], how='left')

dados.shape

# mudar nomes das colunas
# ficar com o código do IBGE
dados = dados.rename(columns={'mun_y': 'mun'})
# remover variáveis desnecessárias
dados = dados.drop(columns=['mun_x'])

# transformar códigos em inteiros
# dados['mun'] = dados['mun'].astype(int)

# visualizar nan em 'mun'
# municípios ainda sem código
df_sem_mun = dados[dados['mun'].isnull()]

"""## Teste com a base de dados do IBGE com distritos

Informações sobre os distritos do Brasil: https://geoftp.ibge.gov.br/organizacao_do_territorio/estrutura_territorial/divisao_territorial/2022/
"""

drive.mount('/content/drive')
os.chdir('/content/drive/My Drive/dados')
br = pd.read_excel('RELATORIO_DTB_BRASIL_DISTRITO.xls', skiprows=6)
# apenas as colunas de interesse
br = br.loc[:, ['UF', 'Nome_UF', 'Código Município Completo', 'Nome_Município', 'Nome_Distrito']]
# substituir códigos pelas siglas
br['UF'] = br['UF'].replace({12: 'AC',
                             27: 'AL',
                             13: 'AM',
                             16: 'AP',
                             29: 'BA',
                             ...
                             17: 'TO'})
# renomear colunas
br.columns = ['uf', 'nome_uf', 'mun', 'nome_mun', 'nome_dist']

# colunas de interesse
br = br.loc[:, ['mun', 'nome_mun', 'uf', 'nome_dist']]

# simplificar os nomes de municípios e distritos
simplificar_nomes(br, 'nome_mun')
simplificar_nomes(br, 'nome_dist')

dados_c = pd.merge(dados, br,
                 left_on=['nome_mun', 'uf'], right_on=['nome_dist', 'uf'],
                 how='left'   # se aqui fosse 'inner', Primavera-SP não entraria
                 )

# mudar nomes das colunas
dados_c = dados_c.rename(columns={'nome_mun_x': 'nome_mun', 'nome_mun_y': 'nome_mun_correto', 'mun_y': 'mun'})

# substituir nome_mun pelos valores de nome_mun_correto onde necessário
# dados_c['nome_mun'] = dados_c['nome_mun_correto'].combine_first(dados_c['nome_mun'])

# remover variáveis desnecessárias
dados_c = dados_c.drop(columns=['nome_dist', 'mun_x'])
# dados_c

# número de apólices sem a informação de código do município
print('Número de apólices com 0 em mun:', len(dados_c[dados_c['mun'] == 0]))
print('Número de códigos de municípios em mun:', len(dados_c['mun'].unique()))
print('Número de NaN em códigos de municípios:', dados_c['mun'].isna().sum())
print('Número de municípios sem código:', len(dados_c[dados_c['mun'].isnull()]['nome_mun'].unique()))

dados_c.columns

# remover nome_mun
dados_c = dados_c.drop(columns=['nome_mun'])
# nome do município correto
dados_c = dados_c.rename(columns={'nome_mun_correto': 'nome_mun'})

dados_c.columns

# reorganizar as colunas
dados_c = dados_c.loc[:, ['apolice', 'mun', 'nome_mun', 'uf', 'seguradora', 'tipo', 'cultura', 'tipo_cultura',
...
# visualizar nan em 'mun'
df_dados_c = dados_c[dados_c['mun'].isnull()]
len(df_dados_c['nome_mun'].unique())
...
# seguradoras - alterar nomes
replacers_seg = {'Aliança do Brasil Seguros S/A.': 'alianca',
...
df['seguradora'] = df['seguradora'].replace(replacers_seg)

# criar nova coluna 'regiao'
# não tem nenhuma observação do AP (Amapá)
df['regiao'] = df['uf'].replace({'TO': 'norte',
                                       'AM': 'norte',
                                       'AC': 'norte',
                                       'RR': 'norte',
                                       'PA': 'norte',
...
df['regiao'] = df['regiao'].replace({'centro-oeste': 'Centro-Oeste',
                                           'sudeste': 'Sudeste',
                                           'sul': 'Sul',
                                           'nordeste': 'Nordeste',
                                           'norte': 'Norte'})

"""Número de observações e variáveis após a criação das novas colunas:"""
...
df.to_excel('/content/drive/My Drive/02-pesquisa/02-orientacoes/2025-wellinton/seg-rural-apo-limpo-23.xlsx', index=False)
